...
Piece TetrisCore::getCurrentPiece()
{
    return currentPiece;
}
...
bool TetrisCore::isGameOver()
{
    return gameOver;
}

void TetrisCore::reset()
{
    score = 0;
    gameOver = false;
...
int TetrisCore::getScore()
{
    return score;
}

bool TetrisCore::hasBlock(int x, int y)
{
    return gameMap[x][y];
}

void TetrisCore::addPiece()
{
    // start position
    srand(time(NULL));
...
void TetrisCore::movePiece(int direction)
{
...
bool TetrisCore::checkPieceCollision()
{
    for (int i = 3; i > -1; i--)
...
void TetrisCore::rotatePiece(bool cw)
{
